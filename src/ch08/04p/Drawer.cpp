 ...
 * @LastEditTime: 2023-04-04 16:33:37
 * @Description:
 * guijiyang@163.com
...
int Drawer::width_{600};
int Drawer::height_{600};
...
void Drawer::init(GLFWwindow *window) {
  ...
  auto aspect = static_cast<float>(width_) / height_;
  setupShdowBuffers(window);
...
// shadow mapping
void Drawer::setupShdowBuffers(GLFWwindow *window) {
...
void Drawer::passTwo() {
  ...
  //  pass window width and height to shader
  // get window width,height and aspect
  glUniform1f(glGetUniformLocation(rendering_program2_, "shadowbufferwidth"), 
              static_cast<float>(width_));
  glUniform1f(glGetUniformLocation(rendering_program2_, "shadowbufferheight"), 
...
void Drawer::run(const char *filename) {
  ...
  auto p_window = glfwCreateWindow(width_, height_, filename, nullptr, nullptr);
