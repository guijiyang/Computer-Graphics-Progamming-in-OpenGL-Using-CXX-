 ...
 * @LastEditTime: 2023-04-04 16:09:07
 * @Description:
 * guijiyang@163.com
...
void Drawer::display(GLFWwindow *window, double current_time) {
  ...
  //  set up light location with rotated axis y.
  auto light_loc = glm::translate(
      glm::mat4(1.0f), glm::vec3(light_loc_.x, light_loc_.y, light_loc_.z));
  auto light_rotate = glm::rotate(
      glm::mat4(1.0f), static_cast<float>(current_time),
      glm::vec3(0.0f, 1.0f, 0.0f));
  light_loc = light_rotate * light_loc;
  light_position_=glm::vec3(light_loc[3]);
  // set up view and prespective matrix from the light of view, for pass 1
  light_v_mat_ =
      glm::lookAt(light_position_,
                  glm::vec3(0.0f), glm::vec3(-light_position_.z, 0.f, light_position_.x));
...
void Drawer::installLights(glm::mat4 view_mat) {
  auto light_pos_v = glm::vec3(
      view_mat * glm::vec4(light_position_, 1.0f));
