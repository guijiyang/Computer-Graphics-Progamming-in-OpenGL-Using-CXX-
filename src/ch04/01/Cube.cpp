...
void Cube::display(GLFWwindow *window, double current_time) {
  ...
  for (int i = 0; i < 24; i++) {
    ...
    // rotations
    auto rmat =
        glm::rotate(glm::mat4(1.0f), 1.75f * static_cast<float>(tf),
                    glm::vec3(0.0f, 1.0f, 0.0f));
    rmat = glm::rotate(rmat, 1.75f * static_cast<float>(tf),
...
void Cube::run() {
  ...
  p_window_ = glfwCreateWindow(600, 600, "4_1", nullptr, nullptr);
