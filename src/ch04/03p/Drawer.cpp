...
void Drawer::setupVertices() {
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(prisma_positions), prisma_positions,
               GL_STATIC_DRAW);
}
...
void Drawer::display(GLFWwindow *window, double current_time) {
  ...
  mv_stack_.top() *= glm::translate(
      ...
      glm::vec3(-sin(static_cast<float>(current_time)) * 4.0,
                cos(static_cast<float>(current_time)) * 4.0, 0.0f));
  mv_stack_.push(mv_stack_.top());
  mv_stack_.top() *=
      glm::rotate(glm::mat4(1.0f), static_cast<float>(current_time),
                  glm::vec3(0.0, 1.0, 0.0));
  ...
  // glEnable(GL_DEPTH_TEST);
  // glDepthFunc(GL_LEQUAL);
  glDrawArrays(GL_TRIANGLES, 0, 24); // draw planet
  mv_stack_.pop();
