 ...
 * @LastEditTime: 2023-04-07 10:28:50
 * @Description:
 * guijiyang@163.com
...
void Drawer::init(GLFWwindow *window) {
  ...
  texture_ids_[0] = setupTexture(texture_1_filename.c_str());
...
void Drawer::setCameraDirection(GLFWwindow *window) {
  ...
  cursor_x_ = cursor_x;
  cursor_y_ = cursor_y;
  // cam_dir_[0]=glm::normalize(cam_dir_[0]);
  // cam_dir_[1]=glm::normalize(cam_dir_[1]);
  // cam_dir_[2]=glm::normalize(cam_dir_[2]);
  if (delta_x == 0.0)
    return;
  {
  ...
  // cam_dir_[0]=glm::normalize(cam_dir_[0]);
  // glm::mat4(glm::vec4(cam_dir_u_.x, cam_dir_u_.y, cam_dir_u_.z, 0.0f),
  //           glm::vec4(cam_dir_v_.x, cam_dir_v_.y, cam_dir_v_.z, 0.0f),
  //           glm::vec4(cam_dir_w_.x, cam_dir_w_.y, cam_dir_w_.z, 0.0f),
  //           glm::vec4(0.f, 0.f, 0.f, 1.0f));
  // cam_dir_u_={camera_direction[0][0], camera_direction[0][1],
  // camera_direction[0][2]}; cam_dir_v_={camera_direction[1][0],
  // camera_direction[1][1], camera_direction[1][2]};
  // cam_dir_w_={camera_direction[2][0], camera_direction[2][1],
...
void Drawer::setCameraLocation(GLFWwindow*window)
{
  if(keybutton_w_pressed_)
  {
    ...
    cam_pos_={cam_pos_.x-direction[0], cam_pos_.y-direction[1], cam_pos_.z-direction[2]};
    el::Loggers::getLogger("Drawer");
    CLOG(INFO, "Drawer")<<"camera position: "<<cam_pos_.x<<" "<<cam_pos_.y<<" "<<cam_pos_.z;
  }
}
...
// void Drawer::keyboardCallback(GLFWwindow *window, int key, int scancode,
//                               int action, int mods) {}
