 ...
 * @LastEditTime: 2023-04-07 15:03:39
 * @Description:
 * guijiyang@163.com
...
void Drawer::init(GLFWwindow *window) {
  ...
  auto texture_filename = config["texture_filename"].as<std::string>();
  ...
  texture_ids_[0] = loadCubeMap(texture_filename.c_str());
...
void Drawer::display(GLFWwindow *window, double current_time) {
  ...
  // prepare to draw the skybox first.
  vmat_ = glm::translate(glm::mat4(1.0f),
                         glm::vec3(-cam_pos_.x, -cam_pos_.y, -cam_pos_.z));
  mvmat_ = vmat_ * glm::rotate(glm::mat4(1.f), static_cast<float>(current_time),
  ...
  mvmat_ = vmat_ *
           ...
           glm::rotate(glm::mat4(1.0f), static_cast<float>(25.f),
  ...
  // calculate torus rotate relative to skybox
  auto position_mat =
      vmat_ *
      glm::translate(glm::mat4(1.0f),
                     glm::vec3(torus_loc_.x, torus_loc_.y, torus_loc_.z)) *
      glm::rotate(glm::mat4(1.f), static_cast<float>(-current_time),
                  glm::vec3(0.f, 1.f, 0.f)) *
      glm::rotate(glm::mat4(1.0f), static_cast<float>(25.f),
                  glm::vec3(1.0f, 0.f, 0.f));
  glUniformMatrix4fv(glGetUniformLocation(rendering_program_,"position_mat"),1,GL_FALSE, glm::value_ptr(position_mat));
  auto inv_mat = glm::transpose(glm::inverse(position_mat));
  glUniformMatrix4fv(glGetUniformLocation(rendering_program_, "norm_mat"), 1,
                     GL_FALSE, glm::value_ptr(inv_mat));
