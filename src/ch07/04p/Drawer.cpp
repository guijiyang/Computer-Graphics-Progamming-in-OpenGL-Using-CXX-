...
void Drawer::installLights() {
  ...
  auto light_pos_v = glm::vec3(
      vmat_ * glm::vec4(light_loc_.x, light_loc_.y, light_loc_.z, 1.0));
  auto light_direction_v =
      vmat_ * glm::vec4(light_direction_.x, light_direction_.y,
                                  light_direction_.z, 1.0);
  light_direction_v=light_direction_v/light_direction_v.w;
  ...
  glUniform3fv(glGetUniformLocation(rendering_program_, "light.direction"), 1,
               glm::value_ptr(glm::vec3(light_direction_v)));
  glUniform1f(glGetUniformLocation(rendering_program_, "light.cutoff"),
              spot_light_cutoff_);
  glUniform1f(glGetUniformLocation(rendering_program_, "light.falloff"),
              spot_light_falloff_);
  glProgramUniform4fv(
      rendering_program_,
